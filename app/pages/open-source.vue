<template>
  <div class="projects">
    ...
    <section
      class="grid grid-cols-1 sm:grid-cols-2 gap-5 md:px-3 mx-auto md:overflow-y-auto overflow-x-hidden md:h-90 md:pt-6"
    >
      <ProjectItem v-for="repo of repos" :key="repo.id" :repo="repo" />
    </section>
  </div>
</template>
...
const loadRepositories = async (page = 1) => {
  return await $fetch<GithubRepo[]>(
    "https://api.github.com/users/acidiney/repos?sort=created&page=" + page
  )
